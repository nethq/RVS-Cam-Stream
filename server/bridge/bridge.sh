...
while true; do
  sqlite3 "$DB" 'SELECT name,udp FROM cams' | while IFS='|' read n u; do
    ...
    echo "[bridge] ingest udp://0.0.0.0:$u  ->  $n , transcoding to -> rtmp://$HOST_IP:1935/$n"
    echo "[bridge] expected hls endpoint from media-mtx ->  http://$HOST_IP:8888/$n/index.m3u8"
    ffmpeg -nostats -loglevel warning -fflags +nobuffer \
      -i "udp://0.0.0.0:$u?listen=1" \
      -c copy -bsf:v h264_mp4toannexb -f flv "rtmp://$HOST_IP:1935/$n" &
  done
  sleep 1
done
